...
Since the root URI, e.g., `http://localhost:8080/` of a web application should show an UI to the user rather then json or xml, the root URI for the rest API should be changed.
...
[source, java]
----
include::res/react/RootUriController.java[]
----

=== Validate

Start the Spring Application, open up the http://localhost:8080/ in a browser and see whether _Hello Spring!_ is shown.
