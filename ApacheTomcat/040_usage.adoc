...
To restart _Tomcat_ you can use the following command.
...
To start Tomcat use `tomcat8.exe` in the bin directory.
...
The available users can be found in the `tomcat-users.xml` file. 
This file is located  of the Tomcat configuration directory, i.e., the `/etc/tomcat/tomcat-users.xml` file under Ubuntu.

On Ubuntu the user for the administrator console is not created automatically.
You have to add the user entry manually to the `/etc/tomcat/tomcat-users.xml`. 
